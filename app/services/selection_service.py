...
class SelectionService:
    ...
    def apply_selection(session_id: str, selected_files: list[str]) -> dict:
        ...
        if not raw.exists():
            # This should never happen if /files works, but return cleanly
            return {
                "ok": False,
        ...
        for rel in selected_files or []:
            # normalize UI paths like "./demo/x.py"
            rel_norm = rel.lstrip("./")
        ...
        return {
            ...
            "copied": copied,
            "skipped": skipped,
            "missing": missing[:50],   # cap to keep response small
            "rejected": rejected[:50],
            "active_file_count": file_count,
            "raw_dir": str(raw_root),
            "active_dir": str(active_root),
        }
